...
while condition:
    ...
    for st in range(stocastic_step):
        ...
        weights, opt_state, cost, grads = optimizer_update(opt_state, weights, X_train[idxs], Y_train[idxs])
        int_grad_list.append(grads[0])

    grads_list.append(np.mean(int_grad_list))
...
#fname_grads = f'Results/grads/grads_{layers}layers_batchsize{batch_size}_stocstep{stocastic_step}_{seed}seed_CE.txt'

#with open(fname_grads,'w') as f:
#    for i in grads_list:
#        f.write(str(i))
#        f.write('\n')
